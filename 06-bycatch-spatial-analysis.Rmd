...
Plot it up
```{r plot-semidi-prib-breeding}
# set up the output
#pdf("pdf_outputs/prib_semidi_breeding_map.pdf", width = 6.5, height = 5.5)
#plot(coastlineWorldFine, clon = -168, clat = mlat, span = 1100, 
pdf("pdf_outputs/semidi_breeding.pdf", width = 5.5, height = 4.5)
...
# add map data
mapPolygon(longitude = semidi.b$long, latitude = semidi.b$lat, lty = 0, col = alpha("darkorange", 0.2))
#mapPolygon(longitude = prib.b$long, latitude = prib.b$lat, lty = 0, col = alpha("red", 0.2))
...
```{r comparison-prib-semidi-nonbreeding}
...
Plot it up
```{r plot-semidi-prib-nonbreeding}
# set up the output
...
```{r histogram}
colors <- c("purple", "red", "darkorange", "dodgerblue")

barplot.df <- colony_perc %>%
  pivot_longer(cols = 5:6, names_to = "type", values_to = "Percentage") %>% # reformat just a bit
  mutate(type = ifelse(type == "pop_perc", "metapopulation", "bycatch")) %>%
  mutate(Percentage = Percentage*100) 

barplot.df$Colony <- as.factor(barplot.df$Colony)


    
pattern.plot <- ggplot(barplot.df, aes(x = Colony, y = Percentage, fill = Colony)) +
  scale_fill_manual(values = alpha(colors, 0.75)) +
  geom_col_pattern(aes(pattern_fill = Colony, pattern_density = type), 
                   colour='black', 
                   pattern = 'stripe',
                   position = 'dodge',
                   size = 0.1,
...
#   ggplot(aes(x = Colony, y = Percentage, fill = type)) +
#   geom_bar(position = "dodge", stat = "identity") +
#   theme_minimal() +
#   #scale_fill_manual(values = c("lightskyblue1", "darkgreen")) +
...
ggsave("pdf_outputs/barplot_stripes.pdf", width = 6, height = 3)
```


Straight colony map
```{r colony-locations-map}
colors <- c("purple", "red", "darkorange", "dodgerblue")[colony_locs$Colony]

#pdf("pdf_outputs/colony_loc_map_colors.pdf", width = 6.5, height = 5.5)
...
# add colony locations
# mapPoints(longitude = colony_locs$lon, latitude = colony_locs$lat, pch = 16, cex = 2, col = alpha("darkgreen", 0.75))

mapPoints(longitude = colony_locs$lon, latitude = colony_locs$lat, pch = 23, col = colors, cex = 1.2, lwd = 2)
